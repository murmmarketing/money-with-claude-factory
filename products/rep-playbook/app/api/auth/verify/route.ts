...
/**
 * Passwordless re-login step 2: verify the code and set the signed session
 * cookie. The code row is consumed on success so it cannot be replayed.
 */
export async function POST(req: Request) {
  ...
  await sb.from('repplaybook_login_codes').update({ consumed: true }).eq('id', row.id);
